...
func update_difficulity_progress_bar() -> void :
	...
	var next_floor_id = curr_state.memory.floor_number
	...
	if latest_cached_floor == -1 or latest_cached_floor != next_floor_id:
		...
		for floor_idx in range(next_floor_id, next_floor_id + 30):
			var max_rooms = gamemode.get_room_count(floor_idx)
			var floors_left = floor_idx - next_floor_id
			for i_room_idx in range(0, max_rooms):
				var next_room = gamemode.get_room_type(
					floor_idx, i_room_idx
				)
				
				for key in rooms_upcoming:
					if next_room == Rooms[key] and rooms_upcoming[key] == -1:
						rooms_upcoming[key] = floors_left
